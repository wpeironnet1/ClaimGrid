...
import {
  ...
  type Bounds,
  type SavedClaimRecord,
  type SavedResearchArea,
  upsertClaimBookmark,
...
export default function ClaimsExplorer() {
  ...
  const [customArea, setCustomArea] = useState<
    (Bounds & { id: string; label: string }) | null
  >(null);
  const [customDraft, setCustomDraft] = useState({
    label: "Custom research area",
    west: "-120",
    south: "38",
    east: "-119",
    north: "39",
  ...
  const area = useMemo(
    () =>
      areaId === "custom" && customArea
        ? customArea
        : (areas.find((item) => item.id === areaId) ?? areas[0]),
    [areaId, customArea],
  );

  useEffect(() => {
    setSaved(parseResearchSnapshots(localStorage.getItem(STORAGE_KEY)));
    setSavedClaims(
      parseClaimBookmarks(localStorage.getItem(CLAIM_STORAGE_KEY)),
    );
  }, []);
  useEffect(() => {
    const controller = new AbortController();
    fetch("/api/health/sources", { signal: controller.signal })
      .then(async (response) => (await response.json()) as SourceHealth)
      ...
      .catch((reason) => {
        if (reason.name !== "AbortError") {
          setSourceHealth({
            ...
            issues: ["Source health could not be verified."],
            freshnessCaveat:
              "No availability or legal conclusion can be drawn without authoritative verification.",
          });
        }
      });
  ...
  const paths = useMemo(
    () =>
      result?.features.flatMap((feature, featureIndex) =>
        rings(feature).map((ring, ringIndex) => {
          const d =
            ring
              .map(([lng, lat], index) => {
                const x = ((lng - area.west) / (area.east - area.west)) * 1000;
                const y =
                  620 - ((lat - area.south) / (area.north - area.south)) * 620;
                return `${index ? "L" : "M"}${x.toFixed(1)} ${y.toFixed(1)}`;
              })
              .join(" ") + " Z";
          const selected = selectedFeatureIndex === featureIndex;
          const label =
            summarizeClaimProperties(feature.properties).claimName ??
            `BLM claim record ${featureIndex + 1}`;
          return (
            <path
              key={`${feature.id ?? featureIndex}-${ringIndex}`}
              d={d}
  ...
  function applyCustomArea(event: FormEvent) {
    ...
    if (!validation.ok) {
      setBoundsError(validation.error);
      return;
    }
    setBoundsError("");
    setCustomArea({
      ...validation.bounds,
      id: "custom",
      label: customDraft.label.trim() || "Custom research area",
    });
    setAreaId("custom");
  }
  ...
  const selectedClaim =
    selectedFeatureIndex === null
      ? null
      : summarizeClaimProperties(
          result?.features[selectedFeatureIndex]?.properties,
        );
  ...
  function saveSelectedClaim() {
    ...
    const record = createClaimBookmark({
      recordKey:
        ...
        String(feature.id ?? `record-${selectedFeatureIndex}`),
      ...
      details: selectedClaim,
      sourceCheckedAt: result.metadata.retrievedAt,
    });
    const next = upsertClaimBookmark(savedClaims, record);
    setSavedClaims(next);
    localStorage.setItem(CLAIM_STORAGE_KEY, JSON.stringify(next));
  }
  function removeSavedClaim(id: string) {
    const next = savedClaims.filter((item) => item.id !== id);
    setSavedClaims(next);
    localStorage.setItem(CLAIM_STORAGE_KEY, JSON.stringify(next));
  }
  ...
  return (
    <>
      <section
        className={`sourceHealth ${sourceHealth?.status ?? "checking"}`}
        aria-live="polite"
      >
        <div>
      ...
      <div className="explorerLayout">
        <section
          className="liveMap"
          aria-label={`BLM active mining claim screening layer for ${area.label}`}
        >
          <div className="mapTop">
            <div>
              <small>OFFICIAL-SOURCE SCREENING VIEW</small>
            ...
            <label>
              ...
              >
                ...
                {customArea && (
                  <option value="custom">{customArea.label}</option>
                )}
              </select>
            </label>
          </div>
          <div className="liveCanvas">
            <svg viewBox="0 0 1000 620" preserveAspectRatio="none">
              <defs>
                ...
                >
                  <path d="M 50 0 L 0 0 0 50" className="gridLine" />
                </pattern>
              </defs>
        ...
        <aside className="resultPanel">
          ...
          <p>
            ...
            not a count of legally available parcels.
          </p>
          {result && (
            <>
              <dl>
                <div>
                  ...
                  <dd>
                    {new Date(result.metadata.retrievedAt).toLocaleString()}
                  </dd>
                </div>
                <div>
                  <dt>Result cap</dt>
              ...
              <div className="screeningWarning">
                <b>Required verification</b>
                <span>{result.metadata.warning}</span>
              </div>
              {selectedClaim && (
                <section className="claimDetail" aria-live="polite">
                  <div>
                    <b>Selected BLM record</b>
                    <button
                  ...
                  <dl>
                    <div>
                      <dt>Case number</dt>
                      <dd>{selectedClaim.caseNumber ?? "Not provided"}</dd>
                    </div>
                    <div>
                      <dt>Disposition</dt>
                      <dd>{selectedClaim.disposition ?? "Not provided"}</dd>
                    </div>
                    <div>
                      <dt>Commodity</dt>
                      <dd>{selectedClaim.commodity ?? "Not provided"}</dd>
                    </div>
                    <div>
                      <dt>Claim type / quality</dt>
                      <dd>{selectedClaim.quality ?? "Not provided"}</dd>
                    </div>
                    <div>
                      <dt>Recorded acres</dt>
                      <dd>{selectedClaim.recordedAcres ?? "Not provided"}</dd>
                    </div>
                    <div>
                      <dt>State</dt>
                      <dd>{selectedClaim.state ?? "Not provided"}</dd>
                    </div>
                  </dl>
                  <p>
                    Attributes are a sanitized screening snapshot. Verify the
                    case file, legal description, status, and accepted location
                    documents in MLRS and with BLM.
                  </p>
                  <button
                    className="saveClaimButton"
                    onClick={saveSelectedClaim}
                  >
                    Save this BLM record
                  </button>
                </section>
              )}
              <button className="saveAreaButton" onClick={saveCurrentArea}>
      ...
      <section className="savedResearch">
        <div>
        ...
        <div className="savedList">
          {saved.length === 0 ? (
            <div className="emptySaved">No areas saved yet.</div>
          ) : (
            saved.map((item) => (
              <article key={item.id}>
                <div>
      ...
      <section className="savedResearch savedClaims">
        <div>
          <span className="stepLabel">SAVED BLM RECORDS</span>
          <h2>Claim bookmarks</h2>
          <p>
            Bookmarks preserve sanitized attributes, viewport bounds, and the
            source-check time. They do not prove current status, boundaries,
            ownership, or availability.
          </p>
        </div>
        <div className="savedList">
          {savedClaims.length === 0 ? (
            <div className="emptySaved">No BLM records saved yet.</div>
          ) : (
            savedClaims.map((item) => (
              <article key={item.id}>
                <div>
                  <b>
                    {item.details.claimName ??
                ...
                <button
                  ...
                  aria-label={`Remove ${item.details.claimName ?? item.details.caseNumber ?? "saved BLM record"}`}
                >
                  Remove
                </button>
              </article>
