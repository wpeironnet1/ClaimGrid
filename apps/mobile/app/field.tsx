...
export default function FieldCapture() {
  ...
  function confirmApproximateReading(accuracy: number | null): Promise<boolean> {
    const detail = accuracy === null ? "The device did not report horizontal accuracy." : `The reported horizontal accuracy is ±${accuracy.toFixed(1)} m.`;
    return new Promise(resolve => Alert.alert(
      "Approximate GPS reading",
      `${detail} Move to a clear view of the sky and recapture when possible. Save this reading only as an approximate field note—not as a boundary, corner, or survey point.`,
      [{ text: "Recapture", style: "cancel", onPress: () => resolve(false) }, { text: "Save approximate", onPress: () => resolve(true) }],
      { cancelable: false }
    ));
  }
  ...
  async function capture() {
    ...
    try {
      ...
      const quality = describeGpsQuality(position.coords.accuracy);
      if ((quality === "weak" || quality === "unknown") && !(await confirmApproximateReading(position.coords.accuracy))) return;
